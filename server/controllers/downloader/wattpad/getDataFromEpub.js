...
const { saveFanficToDB } = require('../helpers')
...
exports.getDataFromEpub = (log, fandomName, paths, collectionName) => {
    console.log(clc.greenBright('[WPD] getPaths()'));
    return new Promise(async function (resolve, reject) {
        await getEpubMetadata(log, fandomName, paths, collectionName);
        resolve();
    })
}
        ...
        epub.on("end", function () {
            epub.getChapter("title_page", async function (err, text) {
                ...
                if (fanfic.Complete) {
                    ...
                    fanfic.FanficID = Number($('h3 a').first().attr('href').split('/story/')[1]);
                    ...
                    await saveFanficToDB(fandomName,fanfic, collectionName);
                }
                resolve();
            });
        });
