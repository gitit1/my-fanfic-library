...
const getDeletedFanfics = async (socket, log, fandom) => {
    ...
    socket && socket.emit('getFanficsData', `<b>Number of New Restored fanfics:</b> <span style="color:purple">${deletedCounters[1]}</span>`);
    socket && socket.emit('getFanficsData', `<b>Number of New Deleted fanfics:</b> <span style="color:red">${deletedCounters[0]}</span>`);
    socket && socket.emit('getFanficsData', `<b>Number of All Deleted fanfics:</b> <span style="color:red">${deletedCounters[2]}</span>`);
    log.info(`Total Number of AO3 fanfics of this fandom: ${fandom.AO3.TotalFanficsInFandom}`);
    log.info(`Number of New Restored fanfics: ${deletedCounters[1]}`);
    log.info(`Number of New Deleted fanfics: ${deletedCounters[0]}`);
    log.info(`Number of All Deleted fanfics: ${deletedCounters[2]}`);
