...
exports.getFiltersRules = async (filters, fandomName, userEmail) => {
    ...
    await filters.map(filter => {
        ...
        switch (filterKey) {
            ...
            case 'title':
                ...
                break;
            case 'series':
                filtersFanficList.push({ 'Series': { $regex: `.*${filterValue.replace('%20', '')}.*`, '$options': 'i' } });
                break;
            case 'categories':
    ...
    let ignoreList = await getIgnoredList(fandomName, userEmail);
