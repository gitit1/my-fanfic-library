...
class Fanfic extends Component {
    ...
    activeFiltersHandler = async (event) => {
        ...
        await onGetFilteredFanfics(fandomName, this.props.userEmail, filterArr, pageLimit, pageNumber).then(() => {
            ...
            this.setState({
                userFanfics, filterArr, pageNumber,
                drawerFilters: false,
                fanficsNumbers: {
                    ...fanficsNumbers,
                    fanficsCurrentCount: fanficsCount
    ...
    cancelFiltersHandler = async () => {
        this.setState({
            filters: filtersArrayInit, filterArr: [], pageNumber: 1, drawerFilters: false,
            urlQueries: { ...this.state.urlQueries, isFiltered: false, filterQuery: '' }
        }, await this.getFanfics)
        this.addUrlQueries()
    }
    ...
    render() {
        ...
        const categoriesProps = {
            inputCategoryFlag, categoriesShowTemp, showSelectCategory, categoriesTemp: categoriesShowTemp,
            getCategories: this.getCategories, saveCategories: this.saveCategories, showCategory: this.showSelectCategoryHandler
        }
        const filtersProps = {
            isManager, drawer: drawerFilters, checked: filters, filterHandler: this.filterHandler,
