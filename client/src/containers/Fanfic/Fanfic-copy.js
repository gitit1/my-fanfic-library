...
class Fanfic extends Component{    
    ...
    componentWillMount(){
        ...
        if(location.search.includes('noUserData') && switches[2].checked){
            let filtersLength = location.search.split('filters=true&')[1].split("&").length - 1;
            if(filtersLength<=1){
                isFiltered = false;
                this.addUrlQueries()
            }
        ...
        if(isFiltered){
            ...
            filterQuery = switches[2].checked ? filterQuery.replace('noUserData','') : filterQuery;

            filterArr =  this.props.location.search.split('filters=true')[1]; 
            filterArr =  switches[2].checked ? filterArr.replace('noUserData','').split('&') : filterArr.split('&')
    ...
    statusHandler = async(fanficId,author,fanficTitle,source,statusType,status,event) =>{
        ...
        const {switches,fandomName,filterArr,pageLimit,pageNumber,fanficsNumbers} = this.state;
        const {userEmail,onStatusHandler,onGetFilteredFanfics} = this.props;
        ...
        switch (statusType) {
            case 'Finished':
                ...
                await onStatusHandler(userEmail,fandomName,fanficId,author,fanficTitle,source,statusType,newStatus)
                flag = true;
                break;
            case 'In Progress':
                if(event.key === 'Enter') {
                    ...
                    await onStatusHandler(userEmail,fandomName,fanficId,author,fanficTitle,source,statusType,newStatus,chapterNum);
                    flag = true;
                }
                break;
            case 'Need to Read':
    ...
    activeFiltersHandler = async(event)=>{
        ...
        if(isFiltered && !event){
            console.log('-----isFiltered:::',this.props.location.search)
    ...
    filterHandler = async(filter,event,type)=>{
        console.log('filter,event,type:',filter,event,type)
        const {filters} = this.state;
        switch (type) {
            case 'source':
    ...
    cancelFiltersHandler = async() =>{
        ...
        if(!switches[2].checked){
            this.setState({filters:{
                filtersArrayInit,
                noUserData:true
            },filterArr:['noUserData'],pageNumber:1,drawerFilters:false,
            urlQueries:{...this.state.urlQueries,isFiltered:true,filterQuery:'&filters=true&noUserData'}},await this.activeFiltersHandler)            
        }else{
    ...
    switchChangeHandler = async (type) =>{
        const   {switches,fandomName,pageLimit,pageNumber,fanficsNumbers,filters,urlQueries} = this.state;
        const   {userEmail,onGetFilteredFanfics} = this.props;
