...
class OtherSitesDownloader extends Component{
    ...
    markStatusHandler = async (fanficId,statusType,event) =>{
      ...
      switch (statusType) {
        case 'Finished':
            ...
            await this.props.onStatusHandler(userEmail,fandomName,fanficId,statusType,newStatus);
            flag=true;
            break;
        case 'In Progress':
            if(event.key === 'Enter') {
                ...
                await this.props.onStatusHandler(userEmail,fandomName,fanficId,statusType,newStatus,chapterNum);
                flag=true;
            }
            break;
        case 'Need to Read':
    ...
    markAsHandler = (fanficId,markType) =>{
      const {userData} = this.state;
      const {userEmail,fandomName} = this.props;

      this.props.onMarkHandler(userEmail,fandomName,fanficId,markType,!userData[markType])
      this.setState({      
        userData:{
          ...userData,
